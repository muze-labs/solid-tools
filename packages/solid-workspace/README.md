...
The first local source factory is `local.memory()`. `graph.resource()` exposes the lower-level source contract for custom storage adapters, and `solid.turtleResource()` names Solid Turtle resources explicitly for source-to-source sync. `workspace.add()` composes one factory-created workspace part or an array of parts and returns the same workspace for chaining.
